...
class Newsitem extends Component {
  render() {
    ...
    return (
      <div className='contanier my-3 mx-8'>
        <div className="card">
        <span className="position-absolute top-0 translate-middle badge rounded-pill bg-danger" style={{right: '0', zIndex:'1'}}>
              {source}
            </span>
          <img
            src={imageUrl?imageUrl:defaultimg}         className="card-img-top"
            alt="Newz"
          />
          <div className="card-body">
            <h5 className="card-title">{title}</h5>
            <p className="card-text">{description}</p>
            <p className="card-text"><small className="text-body-secondary"><h6><b>By {!author? "Unknow":author} on { new Date(date).toUTCString()}</b></h6></small></p>
